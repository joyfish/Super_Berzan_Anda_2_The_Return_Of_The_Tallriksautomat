...
public class Missile {
	...
	public void act() {
		...
		/*
		 * public void act('target'){
		 * 
		 * if (target == inRange()){ 				boolean inRange, area around thrower.
		 * 
		 * position.add(projectileArc('target')); 	projectileArc returns Vector2,
		 * 											calculates the angle to throw projectiles. 
		 * 											Gravity, volocity, targetposition.
		 * 
		 * 
		 * 
		 * }
	...
	public Rectangle getRectangle() {
		return new Rectangle(position.x, position.y, texture.getWidth(),
				texture.getHeight());
	}
